        ...
        try {
            ...
            */
            ...
            for (let i = initialCandle; i < candles.length - 1; i++) {
                ...
                */
               if (sessionParameters.userDefinedParameters.config.fetchBalanceAndOrderHistory !== undefined) {
                    if (sessionParameters.userDefinedParameters.config.fetchBalanceAndOrderHistory === true) {
                        let minuteInterval = 1
                        if (sessionParameters.userDefinedParameters.config.fetchInterval !== undefined) {
                            minuteInterval = sessionParameters.userDefinedParameters.config.fetchInterval
                        }
                        ...
                        if (Date.now() > lastFetch+Math.ceil(60000 * minuteInterval)) {
                            ...
                            let since = Date.now()-300000
                            let limit = undefined
                            let params = undefined
                            
                            orders = await exchangeAPIModuleObject.getOrderHistory(symbol, since, limit, params = {})

                            if (orders !== undefined || orders !== false) {
                            // Store Current raw data from getOrderHistory() at UDV[0][2]
                                tradingEngine.tradingCurrent.tradingEpisode.userDefinedVariables.userDefinedVariable[0][2] = orders
                                SA.logger.info('fetchOrderHistory() -> Stored raw data from fetchOrerHistory() at userDefinedVariable[0][2]')
                            } else {
                                SA.logger.error("fetchOrderHistory() -> Could not retrive order history from the exchange, will retry next cycle.")
                            }
                        }
                    }
                }
