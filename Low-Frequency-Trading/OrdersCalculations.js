        ...
        */
        ...
        if ( order.amount !== undefined) {
            ...
            if (exchangeConfig.options !== undefined) {
                ...
                if (exchangeConfig.options.orderAmountInQuote !== undefined) { orderAmountInQuote = exchangeConfig.options.orderAmountInQuote }
                if (orderAmountInQuote) { tradingEngineOrder.orderBaseAsset.actualSize.value = order.amount / order.average } 
            } 
        ...
        */
        if (order.average !== undefined) {
            ...
            */
            ...
            // Example BTC/USD:BTC -> Calcs must be made for baseAsset in this case because
            // the contract is an inverse contract and is settled in base currency
            // The filling average rate is still the rate
            /*
            if (exchangeConfig.options !== undefined) {
                let orderAmountInQuote = false
                if (exchangeConfig.options.orderAmountInQuote !== undefined) { orderAmountInQuote = exchangeConfig.options.orderAmountInQuote }
                if (orderAmountInQuote) { tradingEngineOrder.orderStatistics.actualRate.value = order.average }
            }*/
            ...
            */
            ...
            if (exchangeConfig.options !== undefined) {
                let orderAmountInQuote = false
                if (exchangeConfig.options.orderAmountInQuote !== undefined) { orderAmountInQuote = exchangeConfig.options.orderAmountInQuote }
                if (orderAmountInQuote) { tradingEngineOrder.orderStatistics.actualRate.value = order.amount / order.price                  
                    // SA.logger.info('ORDER AMOUNT/PRICE CALCS: ' + tradingEngineOrder.orderBaseAsset.actualSize.value)
                }
            }*/
        }            
